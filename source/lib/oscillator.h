...
public:
	...
	struct WaveformParameters
	{
		...
		WaveformParameters() : valueRange(VALUERANGE::RANGE01), invert(false), pulseWidth(0.0), harmonics(0), harmonicInterval(0.0), harmonicIntervalOffset(0.0), filterSlewUp(0.0), filterSlewDown(0.0), customCurve(nullptr)
		{ }

		/// \brief Copy constructor
		WaveformParameters(const WaveformParameters& src) : valueRange(src.valueRange), invert(src.invert), pulseWidth(src.pulseWidth), harmonics(src.harmonics), harmonicInterval(src.harmonicInterval), harmonicIntervalOffset(src.harmonicIntervalOffset), filterSlewUp(src.filterSlewUp), filterSlewDown(src.filterSlewDown), customCurve(src.customCurve)
		{ }

		/// \brief Construct from values
		WaveformParameters(VALUERANGE t_valueRange, Bool t_invert, Float t_pulseWidth, UInt t_harmonics, Float t_harmonicInterval, Float t_harmonicIntervalOffset, Float t_filterSlewUp, Float t_filterSlewDown, SplineData* t_customCurve) : valueRange(t_valueRange), invert(t_invert), pulseWidth(t_pulseWidth), harmonics(t_harmonics), harmonicInterval(t_harmonicInterval), harmonicIntervalOffset(t_harmonicIntervalOffset), filterSlewUp(t_filterSlewUp), filterSlewDown(t_filterSlewDown), customCurve(t_customCurve)
		{ }

		/// \brief Equals operator
		Bool operator ==(const WaveformParameters& c) const
		{
			return valueRange == c.valueRange && invert == c.invert && pulseWidth == c.pulseWidth && harmonics == c.harmonics && harmonicInterval == c.harmonicInterval && harmonicIntervalOffset == c.harmonicIntervalOffset && filterSlewUp == c.filterSlewUp && filterSlewDown == c.filterSlewDown && (customCurve != nullptr && c.customCurve != nullptr) && (EqualSplineDatas(customCurve, c.customCurve));
		}

		/// \brief Not-equals operator
		Bool operator !=(const WaveformParameters& c) const
		{
			return valueRange != c.valueRange || invert != c.invert || pulseWidth != c.pulseWidth || harmonics != c.harmonics || harmonicInterval != c.harmonicInterval || harmonicIntervalOffset != c.harmonicIntervalOffset || filterSlewUp != c.filterSlewUp || filterSlewDown != c.filterSlewDown || (customCurve != c.customCurve) || (!EqualSplineDatas(customCurve, c.customCurve));
		}
	};
...
public:
	...
	BaseBitmap* RenderToBitmap(Int32 w, Int32 h, Oscillator::WAVEFORMTYPE oscType, const WaveformParameters& parameters, UInt32 oversample = 1)
	{
		...
		for (Int32 x = 0; x < wActual; ++x)
		{
			...
			Float y = (Int32)(renderOsc.GetFiltered(renderOsc.SampleWaveform(xSample, oscType, parameters), parameters) * (Float)(hActual1));
	...
	MAXON_ATTRIBUTE_FORCE_INLINE Float GetFiltered(Float value, const WaveformParameters& parameters)
	{
